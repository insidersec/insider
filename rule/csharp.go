...
var CsharpRules []engine.Rule = []engine.Rule{
	...
	Rule{
		OrExpressions: []string{"\\s+var\\s+\\w+\\s*=\\s*\"\\s*\\<\\%\\s*=\\s*\\w+\\%\\>\";", "\\.innerHTML\\s*:\\s*.+"},
		CWE:           "CWE-79",
		AverageCVSS:   5.9,
	...
	Rule{
		AndExpressions: []string{"using\\sSystem\\.Web\\.Mvc;", "using\\sSystem\\.Web;", ".*\\s+:\\s+Controller", ".*Server\\.MapPath\\(\".*\\+"},
		CWE:            "CWE-23",
		AverageCVSS:    5.5,
	...
	Rule{
		OrExpressions: []string{".*\\s+new\\sOdbcCommand\\(.*\".*\\+(?:.*\n*)*.ExecuteReader\\(", ".*\\s+new\\sSqlCommand\\(.*\".*\\+", ".*\\.ExecuteDataSet\\(.*\".*\\+", ".*\\.ExecuteQuery\\(@\".*\\+"},
		CWE:           "CWE-89",
		AverageCVSS:   6.3,
	...
	Rule{
		OrExpressions: []string{"=\\s+new\\s+SHA1CryptoServiceProvider\\(", ":\\s+new\\s+MD5CryptoServiceProvider\\("},
		CWE:           "CWE-326",
		AverageCVSS:   5,
	...
	Rule{
		OrExpressions: []string{"=\\s+new\\s+TripleDESCryptoServiceProvider\\(", ":\\s+new\\s+DESCryptoServiceProvider\\(", ":\\s+TripleDES\\.Create\\(", ":\\s+DES\\.Create\\("},
		CWE:           "CWE-326",
		AverageCVSS:   5,
	...
	Rule{
		ExactMatch:  "requestValidationMode\\s*=\\s*\"(?:4.[1-9]|3.\\d+|2.\\d+|1.\\d+|0.\\d+)\"",
		CWE:         "CWE-20",
		AverageCVSS: 4.6,
	...
	Rule{
		...
		NotOr:       []string{"\\[ValidateAntiForgeryToken\\]"},
		CWE:         "CWE-352",
		AverageCVSS: 6.3,
